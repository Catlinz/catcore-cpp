...
namespace Cat {
	...
	CxProcessPtr CxProcessRunner::getProcess(CxU32 in_id) {
		/* First, check the running processes */
		const CxI32 r_count = m_running.count();
		for (CxI32 i = 0; i < r_count; ++i) {
			if (m_running[i]->id() == in_id) { return CxProcessPtr(m_running[i]); }
		}

		/* Then, check the paused processes */
		const CxI32 p_count = m_paused.count();
		for (CxI32 i = 0; i < p_count; ++i) {
			if (m_paused[i]->id() == in_id) { return CxProcessPtr(m_paused[i]); }
		}

		/* Otherwise, return null pointer (don't check removed) */
		return CxProcessPtr();
	}
