...
namespace cc {
	void testAsyncDataOutputStreamBasicCreateAndDestroy() {
		BEGIN_TEST;
		...
		AsyncDataOutputStream* as = new AsyncDataOutputStream(s);
		assert( !as->canWrite() );
		...
		as = new AsyncDataOutputStream(s);
		assert( as->canWrite() );
		assert( as->getOutputStream() );
	...
	void testAsyncDataOutputStreamClose() {
		BEGIN_TEST;
	...
	void testAsyncDataOutputStreamBasicReadFunctions() {
		BEGIN_TEST;
		...
		assert( s->canWrite() );
		AsyncDataOutputStream* ao = new AsyncDataOutputStream(s);
	...
	void testAsyncDataOutputStreamDataTypeSpecificReadFunctions() {
		BEGIN_TEST;
...
int main(int argc, char** argv) {
	cc::testAsyncDataOutputStreamBasicCreateAndDestroy();
	cc::testAsyncDataOutputStreamClose();
	cc::testAsyncDataOutputStreamBasicReadFunctions();
	cc::testAsyncDataOutputStreamDataTypeSpecificReadFunctions();
	return 0;
}
