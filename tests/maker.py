...
TEST_BASE_DIR = os.path.abspath(FILE_PATH)+"/"
TEST_TYPE = "unit"
...
class Test:
    def __init__(self, testPath):
        # Eg. u/Core/common/CxVector #
        data = [x.strip() for x in testPath.strip("/ ").split("/")]
        self.type = "unit" if data[0] is 'u' else "integration"
        self.module = data[1] if len(data) > 1 else None
        self.group = data[2] if len(data) > 2 else None
        self.file = data[3] if len(data) > 3 else None

        self.codeDir = os.path.normpath(os.getcwd())
        self.platform = "windows" if platform.system().lower() is "windows" else "unix"
        
    def getFilePath():
        return os.path.join(self.getGroupDir(), self.file+"_tests.cpp") if not self.file is None else None

    def getFilePaths():
        if not self.file is None:
            return [self.getFileDir()]
        else:
            group_dirs = self.getGroupDirs()
            file_dirs = []
            for g in group_dirs:
                file_dirs.extend([os.path.join(g, x) for x in os.listdir(g) if x.endswith("._tests.cpp")])
            return file_dirs

    def getGroupDir():
        return os.path.join(self.getModuleDir(), self.group) if not self.group is None else None
    
    def getGroupDirs():
        if not self.group is None:
            return [self.getGroupDir()]
        else:
            module_dirs = self.getModuleDirs()
            group_dirs = []
            for m in module_dirs:
                group_dirs.extend([os.path.join(m, x) for x in os.listdir(m) if not x.startswith("test_")])
            return group_dirs
        
    def getModuleDir():
        return os.path.join(self.getTestDir(), self.module) if not self.module is None else None

    def getModuleDirs():
        if not self.module is None:
            return [self.getModuleDir()]
        else:
            test_dir = getTestDir()
            return [os.path.join(test_dir, x) for x in os.listdir(test_dir)]
    
    def getTestDir():
        return os.path.join(self.codeDir, "tests", self.type)

    def getTests():
        if not self.file is None:
            return [self]
        elif not self.group is None:
            return self.getTestsForGroup(self.module, self.group)
        elif not self.module is None:
            return self.getTestsForModule(self.module)
        else:
            tests = []
            for module in os.listdir(getTestDir()):
                tests.extend(getTestsForModule(module))
            return tests

    def getTestsForGroup(module, group):
        test_str = ('u' if test.type == 'unit' else 'i') + "/" + module + "/" + group + "/"
        tests = []
        for file in os.listdir(os.path.join(self.getTestDir(), module, group)):
            if file.endswith("_tests.cpp"):
                tests.append(Test(test_str + file[0:-10]))
        return tests
                
    def getTestsForModule(module):
        tests = []
        for group in os.listdir(os.path.join(self.getTestDir(), module)):
            tests.extend(getTestsForGroup(module, group))
        return tests
        
    def clean():
        if not self.file is None:
            t_dir = self.getGroupDir()

def getTestsToRun(testPath):
    return Test(testPath).getTests()


def MSBuildProjectFromModule(module):
    

def MSBuildDebug(project, target):
    return ["MSBuild.exe"];

class MSBuild:
    def __init__(self):
        self.cmds = {"release":"/p:Configuration=Release", "debug":"/p:Configuration=Debug;DefineDebug=true", "clean"}

class Maker:
    def __init__(self, testPath):
        data = testPath.strip().split("/")
        self.
        self.codeDir = os.path.normpath(os.getcwd())
        self.testDir = os.path.normpath(self.codeDir+"/tests/"
        self.useMSBuild = platform.system() is "Windows"

        def clean(options):

        def getMSBuildProjectFromModule():
...
def getListOfTestModuleDirs():
        baseDir = getTestDir()
        testDirs = [  os.path.abspath(baseDir+f)+"/" for f in os.listdir(baseDir) if os.path.isdir(baseDir+f) and f != "test_bin" and f != "test_vs" and f != "test_files" ]
        return testDirs
