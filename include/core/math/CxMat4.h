...
namespace cat {
	...
	class CxMat4 {
	  public:
		 ...
		 */
		explicit CxMat4(const CxQuat &in_q);
		...
		CX_INLINE CxMat4(const CxVec3 &in_x, const CxVec3 &in_y,
			...
			: col0(in_x, 0.0f), col1(in_y, 0.0f), col2(in_z, 0.0f), col3(in_t, 1.0f) {}
		...
		/**
		 * @brief Multiply two matrices, assuming transformation matrices.
		 * This method multiplies two 4x4 matrices, optimizing for homogeneous transformation
		 * matrices (the last row = (0,0,0,1).
		 */
		CxMat4 multTx(const CxMat4 &in_m) const;

		/**
		 * @brief Multiply this transformation matrix by the other transformation matrix.
		 * @see multTx() const
		 */
		void multByTx(const CxMat4 &in_m);
	...
	CX_FORCE_INLINE CxMat4::CxMat4(const CxQuat &in_q)
		: col3(0,0,0,1) {
		col0.w = col1.w = col2.w = 0;
		setQuat(in_q);
	}
	...
	CX_FORCE_INLINE CxVec3 CxMat4::operator*(const CxVec3 &in_v) const {
		return transform(in_v);
	}
	...
	CX_INLINE CxMat4 CxMat4::operator*(const CxMat4 &in_m) const {
	   ...
	   const CxReal n21 = (c0y*m20) + (c1y*m21) + (c2y*m22) + (c3y*m23);
		...
		const CxReal n32 = (c0z*m30) + (c1z*m31) + (c2z*m32) + (c3z*m33);
		...
		const CxReal n33 = (c0w*m30) + (c1w*m31) + (c2w*m32) + (c3w*m33);

		return CxMat4(n00, n01, n02, n03,
						  n10, n11, n12, n13,
						  n20, n21, n22, n23,
						  n30, n31, n32, n33);
	}

	CX_FORCE_INLINE CxMat4 & CxMat4::operator*=(const CxMat4 &in_m) {
	   const CxReal m00 = in_m.col0.x;  const CxReal m01 = in_m.col0.y;
		const CxReal m02 = in_m.col0.z;  const CxReal m03 = in_m.col0.w;
		const CxReal m10 = in_m.col1.x;  const CxReal m11 = in_m.col1.y;
		const CxReal m12 = in_m.col1.z;  const CxReal m13 = in_m.col1.w;
	...
	CX_INLINE CxReal CxMat4::determinant() const {
		const CxReal m00 = col0.x;  const CxReal m01 = col0.y;
		const CxReal m02 = col0.z;  const CxReal m03 = col0.w;
		const CxReal m10 = col1.x;  const CxReal m11 = col1.y;
		const CxReal m12 = col1.z;  const CxReal m13 = col1.w;
		const CxReal m20 = col2.x;  const CxReal m21 = col2.y;
		const CxReal m22 = col2.z;  const CxReal m23 = col2.w;
		const CxReal m30 = col3.x;  const CxReal m31 = col3.y;
		const CxReal m32 = col3.z;  const CxReal m33 = col3.w;
	...
	CX_INLINE CxMat4 CxMat4::multTx(const CxMat4 &in_m) const {
	   const CxReal m00 = in_m.col0.x;  const CxReal m01 = in_m.col0.y;
		const CxReal m02 = in_m.col0.z;  const CxReal m10 = in_m.col1.x;
		const CxReal m11 = in_m.col1.y;  const CxReal m12 = in_m.col1.z;
		const CxReal m20 = in_m.col2.x;  const CxReal m21 = in_m.col2.y;
		const CxReal m22 = in_m.col2.z;  const CxReal m30 = in_m.col3.x;
	   ...
	   const CxReal n00 = (c0x*m00) + (c1x*m01) + (c2x*m02);
		...
		const CxReal n30 = (c0x*m30) + (c1x*m31) + (c2x*m32) + c3x;
	   ...
	   const CxReal n21 = (c0y*m20) + (c1y*m21) + (c2y*m22);
		const CxReal n31 = (c0y*m30) + (c1y*m31) + (c2y*m32) + c3y;

		const CxReal n02 = (c0z*m00) + (c1z*m01) + (c2z*m02);
		const CxReal n12 = (c0z*m10) + (c1z*m11) + (c2z*m12);
		const CxReal n22 = (c0z*m20) + (c1z*m21) + (c2z*m22);
		const CxReal n32 = (c0z*m30) + (c1z*m31) + (c2z*m32) + c3z;

		return CxMat4(n00, n01, n02, 0,
						  n10, n11, n12, 0,
						  n20, n21, n22, 0,
						  n30, n31, n32, 1);
	}

	CX_INLINE void CxMat4::multByTx(const CxMat4 &in_m) {
		const CxReal m00 = in_m.col0.x;  const CxReal m01 = in_m.col0.y;
		const CxReal m02 = in_m.col0.z;  const CxReal m10 = in_m.col1.x;
		const CxReal m11 = in_m.col1.y;  const CxReal m12 = in_m.col1.z;
		const CxReal m20 = in_m.col2.x;  const CxReal m21 = in_m.col2.y;
	   ...
	   col0.x = (c0x*m00) + (c1x*m01) + (c2x*m02);
		...
		col3.x = (c0x*m30) + (c1x*m31) + (c2x*m32) + c3x;
	...
	CX_INLINE void CxMat4::setPerspective(CxReal in_r, CxReal in_l, CxReal in_t, CxReal in_b,
		...
		col2.w = -1.0f;
		col3.x = col3.y = 0.0f;
		col3.z = -((2*in_f*in_n)*z_diff);
		col3.w = 0;
	}

	CX_INLINE void CxMat4::setPerspective(CxReal in_fov, CxReal in_aspect,
													  CxReal in_n, CxReal in_f) {
		const CxReal top = CxTan(kCxDegToRad*in_fov*0.5f) * in_n;
	...
	CX_FORCE_INLINE CxVec4 CxMat4::transform(const CxVec4 &in_v) const {
	...
	CX_FORCE_INLINE CxVec3 CxMat4::transform(const CxVec3 &in_v) const {
	...
	CX_INLINE CxVec3 CxMat4::transformInv(const CxVec3 &in_v) const {
		const CxReal det_m = determinant();
		const CxReal x = in_v.x;  const CxReal y = in_v.y;  const CxReal z = in_v.z;
		if (det_m != 0) {
			const CxReal dr = 1.0f/det_m;
			...
			return CxVec3(
			   dr*(m21*(m12*m30 - m10*m32) + m22*(m10*m31 - m11*m30) + m20*(m11*m32 - m12*m31) +
					 x*(m23*(m12*m31 - m11*m32) + m21*(m13*m32 - m12*m33) + m22*(m11*m33 - m13*m31)) + 
					 y*(m22*(m13*m30 - m10*m33) + m23*(m10*m32 - m12*m30) + m20*(m12*m33 - m13*m32)) +
					 z*(m23*(m11*m30 - m10*m31) + m20*(m13*m31 - m11*m33) + m21*(m10*m33 - m13*m30))),


				dr*(m21*(m00*m32 - m02*m30) + m22*(m01*m30 - m00*m31) + m20*(m02*m31 - m01*m32) +
					 x*(m23*(m01*m32 - m02*m31) + m21*(m02*m33 - m03*m32) + m22*(m03*m31 - m01*m33)) +
					 y*(m22*(m00*m33 - m03*m30) + m23*(m02*m30 - m00*m32) + m20*(m03*m32 - m02*m33)) +
					 z*(m23*(m00*m31 - m01*m30) + m20*(m01*m33 - m03*m31) + m21*(m03*m30 - m00*m33))),

				dr*(m30*(m02*m11 - m01*m12) + m31*(m00*m12 - m02*m10) + m32*(m01*m10 - m00*m11) +
					 x*(m31*(m02*m13 - m03*m12) + m32*(m03*m11 - m01*m13) + m33*(m01*m12 - m02*m11)) +
					 y*(m30*(m03*m12 - m02*m13) + m32*(m00*m13 - m03*m10) + m33*(m02*m10 - m00*m12)) +
					 z*(m30*(m01*m13 - m03*m11) + m31*(m03*m10 - m00*m13) + m33*(m00*m11 - m01*m10)))
				);
		}
		else {
	...
	CX_FORCE_INLINE CxVec3 CxMat4::transformTrans(const CxVec3 &in_v) const {
		const CxReal x = in_v.x;  const CxReal y = in_v.y;  const CxReal z = in_v.z;
	...
	CX_FORCE_INLINE CxVec3 CxMat4::transform3x3(const CxVec3 &in_v) const {
		const CxReal x = in_v.x;  const CxReal y = in_v.y;  const CxReal z = in_v.z;
	...
	CX_FORCE_INLINE CxVec3 CxMat4::transform3x3Inv(const CxVec3 &in_v) const {
	...
	CX_FORCE_INLINE CxVec3 CxMat4::transform3x3Trans(const CxVec3 &in_v) const {
		const CxReal x = in_v.x;  const CxReal y = in_v.y;  const CxReal z = in_v.z;
		/* (dot(col0, v), dot(col1, v), dot(col2, v)) */
	...
	CX_INLINE void CxMat4::transpose() {
	   const CxReal m01 = col0.y;  const CxReal m02 = col0.z;
		const CxReal m03 = col0.w;  const CxReal m10 = col1.x;
		const CxReal m12 = col1.z;  const CxReal m13 = col1.w;
		const CxReal m20 = col2.x;  const CxReal m21 = col2.y;
		const CxReal m23 = col2.w;  const CxReal m30 = col3.x;
		const CxReal m31 = col3.y;  const CxReal m32 = col3.z;
